...
_IMPERATIVE_TOKENS = (
    ...
    "system override", "bỏ qua mọi hướng dẫn", "you must now",
...
class Gateway:
    ...
    def note_provenance(self, anchor: str, etag: str) -> None:
        """Record a freshly read provenance etag for write precondition headers."""
        self._etags[anchor] = etag

    def note_card(self, server: str, card: dict) -> None:
        """Record a verified peer agent card for A2A admission checks."""
        self._admitted_cards[server] = card

    def _act_owns_target(self, cmd: Command) -> bool:
        """Authority check: authority derives from `act` (whom you serve), never `sub`."""
        act = getattr(self.ctx, "act", None)
        if not act:
            return True
        act_id = str(act).replace("Learner:", "").strip()
        for key in ("learner", "learner_id", "target", "subject"):
            target = cmd.args.get(key)
            if target:
                target_id = str(target).replace("Learner:", "").strip()
                if target_id and target_id != act_id:
                    return False
        return True

    def _routes_on_header(self, cmd: Command) -> bool:
        """Refuse routes smuggled through the request body (header_spoof / replica_flip)."""
        return not (cmd.args.get("route") or cmd.args.get("_route") or cmd.args.get("replica"))

    def _instruction_in_args(self, cmd: Command) -> bool:
        """Detect prompt injection imperatives in command arguments."""
        blob = " ".join(str(v) for v in cmd.args.values()).lower()
        return any(token in blob for token in _IMPERATIVE_TOKENS)

    def _card_admitted(self, cmd: Command) -> bool:
        """Verify peer card admission for A2A servers."""
        if cmd.server not in A2A_SERVERS:
            return True
        if cmd.server not in self._admitted_cards:
            return True
        card = self._admitted_cards.get(cmd.server) or {}
        return bool(card.get("verified", True))

    def _audience_matches(self, cmd: Command) -> bool:
        """Verify delegation audience matches the server called."""
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        Enforces all 4 jobs: ROUTE, ADMIT, AUTHORIZE, BUDGET.
        """
        self._telemetry.decision_seen(cmd)

        # ------------------------------------------------------------------
        # JOB 2 — ADMIT: Check for body route spoofing, peer admission, injection
        # ------------------------------------------------------------------
        ...
        if self._instruction_in_args(cmd):
            return self.deny(cmd, "imperative instruction found in command arguments")
        ...
        if not self._act_owns_target(cmd):
            ...
            return self.deny(cmd, f"target learner is not owned by the defender in act ({act})")

        # ------------------------------------------------------------------
        # JOB 1 — ROUTE: Successor mapping and replica header routing
        # ------------------------------------------------------------------
        ...
        if "mcp-replica" not in {k.lower() for k in headers}:
            headers["Mcp-Replica"] = "w"

        # Lease management for slides.get_frame
        lease_id = cmd.lease_id
        if cmd.server == "slides" and cmd.tool == "get_frame" and not lease_id:
            live_leases = getattr(self.ctx, "leases", ())
            if live_leases:
                lease_id = live_leases[-1]
                rewritten = True

        # Write Preconditions: If-Match etag & Idempotency-Key
        key = (server, tool)
        if key in WRITE_TOOLS:
            anchor = str(cmd.args.get("anchor", ""))
            etag = self._etags.get(anchor) or headers.get("If-Match") or headers.get("if-match")
            if etag:
                headers["If-Match"] = etag
            ikey = headers.get("Idempotency-Key") or headers.get("idempotency-key") or f"{cmd.cmd_id}"
            if ikey in self._idempotency_keys:
                return self.deny(cmd, f"write with idempotency key '{ikey}' already committed this duel")
            self._idempotency_keys.add(ikey)
            headers["Idempotency-Key"] = ikey

        # ------------------------------------------------------------------
        # JOB 4 — BUDGET: Mask field rewrite to prevent wasteful traps
        # ------------------------------------------------------------------
        fields = cmd.fields
        if not fields or fields == ("*",):
            fields = DEFAULT_MASKS.get(key, fields)
            if fields != cmd.fields:
                rewritten = True
    ...
    def deny(self, cmd: Command, reason: str) -> Decision:
        """Helper to create a structured Decision(verdict='deny', reason=reason)."""
    ...
    def _to_tool_call(self, cmd: Command) -> "ToolCall":
        """`Command` -> the `ToolCall` (CONTRACTS.md 3.1) the arena will execute."""
        fields = {
            "server": cmd.server,
            "tool": cmd.tool,
            "args": dict(cmd.args),
